 ...
 * @class Rat.Loader
 *
 * Rat.Loader is the heart of the new dynamic dependency loading capability in Rat JS 4+. It is most commonly used
 * via the {@link Rat#require} shorthand. Rat.Loader supports both asynchronous and synchronous loading
 ...
 *
 *     // Syntax
 *     Rat.require({String/Array} expressions);
 *
 *     // Example: Single alias
 *     Rat.require('widget.window');
 *
 *     // Example: Single class name
 *     Rat.require('Rat.window.Window');
 *
 *     // Example: Multiple aliases / class names mix
 *     Rat.require(['widget.window', 'layout.border', 'Rat.data.Connection']);
 *
 *     // Wildcards
 *     Rat.require(['widget.*', 'layout.*', 'Rat.data.*']);
 *
 * ### Method 2: Explicitly exclude what you don't need: ###
 *
 *     // Syntax: Note that it must be in this chaining format.
 *     Rat.exclude({String/Array} expressions)
 *        .require({String/Array} expressions);
 *
 *     // Include everything except Rat.data.*
 *     Rat.exclude('Rat.data.*').require('*');
 *
 *     // Include all widgets except widget.checkbox*,
 *     // which will match widget.checkbox, widget.checkboxfield, widget.checkboxgroup, etc.
 *     Rat.exclude('widget.checkbox*').require('widget.*');
 *
 ...
 *
 * There's one simple rule to follow: Instantiate everything with Rat.create instead of the `new` keyword
 *
 *     Rat.create('widget.window', { ... }); // Instead of new Rat.window.Window({...});
 *
 *     Rat.create('Rat.window.Window', {}); // Same as above, using full class name instead of alias
 *
 *     Rat.widget('window', {}); // Same as above, all you need is the traditional `xtype`
 *
 * Behind the scene, {@link Rat.ClassManager} will automatically check whether the given class name / alias has already
 *  existed on the page. If it's not, Rat.Loader will immediately switch itself to synchronous mode and automatic load the given
 ...
 *
 * Rat.Loader will automatically fetch all dependencies on demand as they're needed during run-time. For example:
 *
 *     Rat.onReady(function(){
 *         var window = Rat.createWidget('window', {
 *             width: 500,
 *             height: 300,
 *             layout: {
 *                 type: 'border',
 ...
 *
 *     [Rat.Loader] Synchronously loading 'Rat.window.Window'; consider adding Rat.require('Rat.window.Window') before your application's code
 *     ClassManager.js:432
 *     [Rat.Loader] Synchronously loading 'Rat.layout.container.Border'; consider adding Rat.require('Rat.layout.container.Border') before your application's code
 *
 * Simply copy and paste the suggested code above `Rat.onReady`, i.e:
 *
 *     Rat.require('Rat.window.Window');
 *     Rat.require('Rat.layout.container.Border');
 *
 *     Rat.onReady(...);
 *
 ...
 * During production, all dependencies should be combined into one single JavaScript file. Rat.Loader makes
 * the whole process of transitioning from / to between development / maintenance and production as easy as
 * possible. Internally {@link Rat.Loader#history Rat.Loader.history} maintains the list of all dependencies your application
 * needs in the exact loading sequence. It's as simple as concatenating all files in this array into one,
 * then include it on top of your application.
 *
 * This process will be automated with Sencha Command, to be released and documented towards Rat JS 4 Final.
 *
 * @singleton
 */

Rat.Loader = new function() {
    var Loader = this,
        Manager = Rat.ClassManager,
        Class = Rat.Class,
        flexSetter = Rat.Function.flexSetter,
        alias = Rat.Function.alias,
        pass = Rat.Function.pass,
        defer = Rat.Function.defer,
        arrayErase = Rat.Array.erase,
        //<if nonBrowser>
    ...
    Rat.apply(Loader, {
        ...
        config: {
             ...
             *
             *     {
             *         'Rat': '.', // This is set by default, Rat.layout.container.Container will be
         ...
         * is included in the page, and before Rat.onReady. i.e:
         *
         *     <script type="text/javascript" src="ext-core-debug.js"></script>
         *     <script type="text/javascript">
         *         Rat.Loader.setConfig({
         *           enabled: true,
         *           paths: {
         *               'My': 'my_own_path'
         *           }
         ...
         *     </script>
         *
         * Refer to config options of {@link Rat.Loader} for the list of possible properties
         *
         * @param {Object} config The config object to override the default values
         * @return {Rat.Loader} this
         */
        setConfig: function(name, value) {
            if (Rat.isObject(name) && arguments.length === 1) {
                Rat.merge(Loader.config, name);
            }
            else {
                Loader.config[name] = (Rat.isObject(value)) ? Rat.merge(Loader.config[name], value) : value;
            }
         ...
         * For Example:
         *
         *     Rat.Loader.setPath('Rat', '.');
         *
         * @param {String/Object} name See {@link Rat.Function#flexSetter flexSetter}
         * @param {String} path See {@link Rat.Function#flexSetter flexSetter}
         * @return {Rat.Loader} this
         * @method
         */
         ...
         *
         *     Rat.Loader.setPath('My', '/path/to/My');
         *
         *     alert(Rat.Loader.getPath('My.awesome.Class')); // alerts '/path/to/My/awesome/Class.js'
         *
         ...
         *     });
         *
         *     alert(Rat.Loader.getPath('My.awesome.Class')); // alerts '/other/path/for/awesome/stuff/Class.js'
         *
         *     alert(Rat.Loader.getPath('My.awesome.more.Class')); // alerts '/more/awesome/path/Class.js'
         *
         *     alert(Rat.Loader.getPath('My.cool.Class')); // alerts '/path/to/lib/cool/Class.js'
         *
         *     alert(Rat.Loader.getPath('Unknown.strange.Stuff')); // alerts 'Unknown/strange/Stuff.js'
         *
         ...
         * finishes, within the optional scope. This method is aliased by {@link Rat#require Rat.require} for convenience
        ...
        /**
         * Synchronously loads all classes by the given names and all their direct dependencies; optionally executes the given callback function when finishes, within the optional scope. This method is aliased by {@link Rat#syncRequire} for convenience
         * @param {String/Array} expressions Can either be a string or an array of string
         * @param {Function} fn (Optional) The callback function
         * @param {Object} scope (Optional) The execution scope (`this`) of the callback function
         * @param {String/Array} excludes (Optional) Classes to be excluded, useful when being used with expressions
         ...
         *     Rat.exclude('Rat.data.*').require('*');
         *
         *     Rat.exclude('widget.button*').require('widget.*');
         *
        ...
        onReady: function(fn, scope, withDomReady, options) {
            ...
            if (withDomReady !== false && Rat.onDocumentReady) {
                oldFn = fn;

                fn = function() {
                    Rat.onDocumentReady(oldFn, scope, options);
                };
            }
    ...
    Rat.apply(Loader, {
        ...
        cleanupScriptElement: function(script, remove, collect) {
            ...
            script.onload = script.onreadystatechange = script.onerror = null;
            if (remove) {
                Rat.removeNode(script);       // Remove, since its useless now
                if (collect) {
                    for (prop in script) {
                        try {
                            script[prop] = null;
         ...
         * is called before {@link Rat#isReady}, the script's load will delay the transition
         * to ready. This can be used to load arbitrary scripts that may contain further
         * {@link Rat#require Rat.require} calls.
         *
        ...
        loadScript: function (options) {
            ...
            src = config.disableCaching ?
                (url + '?' + config.disableCachingParam + '=' + Rat.Date.now()) : url;
        ...
        loadScriptFile: function(url, onLoad, onError, scope, synchronous) {
            ...
            var config = Loader.getConfig(),
                noCacheUrl = url + (config.disableCaching ? ('?' + config.disableCachingParam + '=' + Rat.Date.now()) : ''),
            ...
            if (!synchronous) {
                ...
                if (isCrossOriginRestricted
                    ...
                    onError.call(Loader, "Failed loading synchronously via XHR: '" + url + "'; It's likely that the file is either " +
                                       ...
                                       "Rat.require instead.", synchronous);
                    //</debug>
                }
                    ...
                    Rat.globalEval(xhr.responseText + "\n//@ sourceURL=" + url);
        ...
        require: function(expressions, fn, scope, excludes) {
            ...
            if (fn) {
                ...
                callback = Rat.emptyFn;
            }

            scope = scope || Rat.global;
            ...
            if (classNames.length > 0) {
                if (!Loader.config.enabled) {
                    throw new Error("Rat.Loader is not enabled, so dependencies cannot be resolved dynamically. " +
        ...
        onFileLoaded: function(className, filePath) {
            ...
            if (!Loader.syncModeEnabled && Loader.numPendingFiles === 0 && Loader.isLoading && !Loader.hasFileLoadError) {
                ...
                missingClasses = Rat.Array.filter(Rat.Array.unique(missingClasses), function(item) {
        ...
        onFileLoadError: function(className, filePath, errorMessage, isSynchronous) {
            ...
            //<debug error>
            throw new Error("[Rat.Loader] " + errorMessage);
            //</debug>
        },
        ...
        addUsedClasses: function (classes) {
            ...
            if (classes) {
                ...
                for (i = 0, ln = classes.length; i < ln; i++) {
                    cls = classes[i];
                    if (typeof cls == 'string' && !Rat.Array.contains(usedClasses, cls)) {
     ...
     * @private
     */
    Rat.disableCacheBuster = function (disable, path) {
        var date = new Date();
    ...
    /**
     * Convenient alias of {@link Rat.Loader#require}. Please see the introduction documentation of
     * {@link Rat.Loader} for examples.
     * @member Rat
     * @method require
     */
    Rat.require = alias(Loader, 'require');

    /**
     * Synchronous version of {@link Rat#require}, convenient alias of {@link Rat.Loader#syncRequire}.
     *
     * @member Rat
     * @method syncRequire
     */
    Rat.syncRequire = alias(Loader, 'syncRequire');

    /**
     * Convenient shortcut to {@link Rat.Loader#exclude}
     * @member Rat
     * @method exclude
     */
    Rat.exclude = alias(Loader, 'exclude');

    /**
     * @member Rat
     * @method onReady
     */
    Rat.onReady = function(fn, scope, options) {
    ...
    Class.registerPreprocessor('loader', function(cls, data, hooks, continueFn) {
        ...
        {
              extend: 'Rat.MyClass',
              requires: ['Rat.some.OtherClass'],
              mixins: {
                  observable: 'Rat.util.Observable';
              }
        }
        ...
        */
        ...
        if (className) {
            ...
            detectDeadlock = function(cls) {
                ...
                if (requiresMap[cls]) {
                    if (Rat.Array.contains(requiresMap[cls], className)) {
     ...
     * this class is created, but are guaranteed to be available before Rat.onReady listeners are
     * invoked. For example:
     *
     *     Rat.define('Mother', {
     *         uses: ['Child'],
     ...
     *             // return new Child();
     *
     *             // Instead use Rat.create() to load the class at the spot if not loaded already:
     *             return Rat.create('Child');
     *         }
     *     });
     */
